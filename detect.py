...
# from genocr import saved_model_loaded
from main import interpreter

flags.DEFINE_string('framework', 'tflite', '(tf, tflite')
flags.DEFINE_string('weights', './checkpoints/yolov4-416-fp32.tflite','path to weights file')
...
flags.DEFINE_boolean('gen_ocr', True, 'perform text recognition')
flags.DEFINE_string('ocr_out', './detections/ocr_output.txt', 'path to ocr text results')
...
def main(_argv):
    ...
    for count, image_path in enumerate(images, 1):
        ...
        # print("boxes: ", boxes, scores, classes)
        ...
        if FLAGS.count:
            ...
            image = utils.draw_bbox(original_image, pred_bbox, FLAGS.info, counted_classes, allowed_classes=allowed_classes, read_plate = FLAGS.gen_ocr, ocr_file=FLAGS.ocr_out)
        else:
            image = utils.draw_bbox(original_image, pred_bbox, FLAGS.info, allowed_classes=allowed_classes, read_plate = FLAGS.gen_ocr, ocr_file=FLAGS.ocr_out)
